...
describe("mVault", function () {
    ...
    beforeEach(async () => {
        ...
        this.Strategy = await Strategy.new(this.mockERC20.address);
        this.Strategy2 = await Strategy.new(this.mockERC2.address);
    ...
    context('During the first decay period, ERC/LP tokens are added to the field', () => {
        ...
        it("should have correct withdraw", async () => {
            //approve 合约可操作的资产
            // expect(await this.mVault.balance({from: bob})).to.be.bignumber.equal("900");
            // await this.mController.balanceOf(this.mockERC20.address)
            // await this.mVault.balance()
            // await this.mockERC20.balanceOf({from: bob})
